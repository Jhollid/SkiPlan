...
1. As a user, I want to select a ski resort from a list of supported European Alpine resorts, so that I can plan my day at the correct mountain.
2. As a user, I want to see a rendered map of the selected resort showing all pistes, lifts, villages, and points of interest, so that I can visually understand the resort layout.
3. As a user, I want pistes on the map to be colour-coded by difficulty (green/blue/red/black), so that I can quickly identify which runs suit my level.
4. As a user, I want to see which lifts are currently open or closed on the map, so that I can account for closures in my planning.
5. As a user, I want closed lifts to be visually distinct (greyed out or marked), so that I don't accidentally plan a route that relies on a closed lift.
6. As a user, I want the resort map to be available offline once downloaded, so that I can reference it in areas with no mobile signal.

### Route Planning

1. As a user, I want to set a starting point on the resort map (a lift station, top of a run, or village), so that my route plan begins from where I actually am.
2. As a user, I want to set a destination (a village, a restaurant, a lift, or a point on the map), so that the app plans a route to get me there.
3. As a user, I want the app to generate a step-by-step plan of named pistes and lifts to reach my destination, so that I know exactly what to ski in what order.
4. As a user, I want the app to respect my difficulty preferences when generating a route, so that I am never routed down a slope harder than my selected maximum.
5. As a user, I want the app to respect my lift type exclusions when generating a route, so that routes never include lift types I have opted out of.
6. As a user, I want the app to apply my routing style preference (fastest vs. most enjoyable) when generating a route, so that the plan fits my priorities for the day.
7. As a user, I want the option to regenerate or adjust a route if I am not happy with the first suggestion, so that I have control over my plan.
8. As a user, I want to mark specific named runs as favourites, so that the app prioritises including them in planned routes where they logically fit.
9. As a user, I want to see an overview of my planned route before committing to it (total number of runs, lifts, approximate vertical drop), so that I can decide if it suits my goals.
10. As a user, I want the planned route to avoid repeating the same run where possible if I have enabled that preference, so that my day is varied.
11. As a user, I want to be able to plan multiple sequential routes in a day (e.g. morning run to lunch village, afternoon run to end village), so that I can structure my full day on the mountain.

### Offline & Downloadable Plans

1. As a user, I want to download a route plan before leaving the hotel so it is accessible without mobile signal, so that I can follow the plan on the slope.
2. As a user, I want to see a static image of the resort piste map with my planned route highlighted on it, so that I can reference it as I ski without needing to interact with the app.
3. As a user, I want the steps of my planned route to be numbered on the static map image and in the step list, so that I can cross-reference them easily.
4. As a user, I want to download the resort piste map for offline use, so that I can browse the full map without signal.

### Groups & Friend Coordination

1. As a user, I want to create a trip group and invite friends by their app username or email, so that we are connected within the app for the duration of our ski trip.
2. As a user, I want the group to have a maximum active duration of one week, after which it automatically disbands, so that location sharing does not persist beyond the trip.
3. As a user, I want to receive a notification when someone invites me to a group, so that I can accept or decline promptly.
4. As a user, I want to see a list of all members of my current group, so that I know who I am connected with.
5. As a user, I want to ping a friend's location on-demand, so that I can request their current GPS position without continuous tracking.
6. As a user, I want to be notified when a friend pings my location, so that I am aware my position has been shared.
7. As a user, I want to see a friend's last known location plotted on the resort map, so that I have a visual sense of where they are on the mountain.
8. As a user, I want to use "Take Me to Them" for any friend in my group, so that the app plots a step-by-step route from my location to theirs.
9. As a user, I want the "Take Me to Them" route to respect my difficulty and lift preferences, so that I am not routed down unsuitable terrain to reach a friend.
10. As a user, I want to use "Meet in the Middle" for any friend in my group, so that the app suggests a logical midpoint both of us can ski to.
11. As a user, I want the "Meet in the Middle" suggestion to default to the quickest meeting point (either a lift base or a village), so that we minimise waiting time.
12. As a user, I want to be able to choose between a village or a lift station as the meeting point in "Meet in the Middle", so that we can agree to wait somewhere comfortable.
13. As a user, I want both parties to receive the route plan when a "Meet in the Middle" is calculated, so that we each know how to get there.
14. As a user, I want to leave a group manually before the week expires, so that I can stop sharing my location if my plans change.
15. As a user, I want the group and all associated location data to be permanently deleted after the group disbands, so that my privacy is protected.

### Venue Discovery & Advertising

1. As a user, I want to see on-slope restaurants, bars, and businesses marked on the resort map, so that I can discover options for food and drink.
2. As a user, I want to tap on a venue marker to see its name, a short description, and any current promotions or events, so that I can decide if I want to visit.
3. As a user, I want to use "Take Me There" on any venue to get a step-by-step route to that business, so that I can plan my route to it efficiently.
4. As a user, I want venue routes to respect my difficulty and lift preferences, so that the route to a restaurant is no harder than I am comfortable with.
5. As a user, I want to see sponsored venue cards in the app feed, so that I can discover businesses that are actively promoting themselves.
6. As a resort business owner, I want to create a listing for my venue within the app, so that skiers can discover my restaurant, bar, or event.
7. As a resort business owner, I want to post events or daily specials that appear in the venue detail view, so that I can attract skiers on the day.
